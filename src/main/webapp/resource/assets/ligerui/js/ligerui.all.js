function jsonToParm(parms) {
	var parm = "";
    for (var i = 0, l = parms.length; i < l; i++) {
        var tmp = parms[i];
        parm += "&" + encodeURIComponent(tmp.name) + "=" + encodeURIComponent(tmp.value);
    }
    return parm;
}
function trim(str) {
...
//函数名：checkLen
//功能介绍：检查字符串(包含汉字,一个汉字为2位长度)的长度
//参数说明：要检查的字符串
//       限制的长度 
//返回值：大于限制长度返回为false，小于等于限制长度返回为true 
        ...
        var defaultOp = {
            ...
            { 
                ...
                if (editor.setValue)
                {
                    ...
                    // 设置了isRefMul属性-
					// 获取到[[id1,value1],[id2,value2]]的形式，需要合并为一个完整字符串
...
{
    /*
	 * 以html的方式加载组件 程序会查询以 liger-插件名 类名的Dom,从dom加载相应的参数并调用插件 比如遇到 .liger-grid
	 * 的dom，会找到 liger.defaults.Grid 加载需要的参数
	 * 而在config.Grid中配置了这些参数的类型,会动态得加载data,而columns会设置为数组 参数处理的优先级为： 1,ignores
	 * 忽略不处理的参数 2,dom存在 {属性名} 的类名 ,比如 <ul class="columns"></ul>
	 * ,便会将这个参数设置为复杂属性(object或array):找到相应的defaults和config来加载
	 * defaults是先找$.liger.inject.defaults,找不到再找liger.defaults的
	 * config为{父配置}.{属性名},比如 config.Grid.columns 3,直接读取 data-{属性名} 或者 {属性名}
	 * 的dom属性
	 */
    liger.inject = {
		 ...
		 * 获取规则：获取default时会先找这里,找不到再找liger.defaults,比如
		 * liger.defaults.Grid_columns 备注：这里只定义了参数的列表
		 */
        defaults: {
            Grid_detail: {
                height: null,
...
(function ($)
{
    ...
    $.ligerui.controls.CheckBoxList.ligerExtend($.ligerui.controls.Input, {
        ...
        _setUrl: function (url) {
			...
			// begin ylc
            if((/[\?]/g).test(url)){
            	url+="&randomsn="+Math.random().toString().slice(2);
            }else{
            	url+="?randomsn="+Math.random().toString().slice(2);
            }
            // end ylc
...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
		...
		clickSelectHide:false,  //点击选择隐藏ylc
        initValue: null,
		value : null,
	    initText: null,
		valueField: 'id',
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _changeValue: function (newValue, newText,isSelectEvent)
        {
            ...
            if(p.clickSelectHide)
            	g._toggleSelectBox(true);//点击选择隐藏ylc
        },
        // 更新选中的值(复选框)
        ...
        { 
            ...
            // begin ylc            
            if((/[\?]/g).test(p.detailUrl)){
            	p.detailUrl+="&randomsn="+Math.random().toString().slice(2);
            }else{
            	p.detailUrl+="?randomsn="+Math.random().toString().slice(2);
            }
            //end ylc
        ...
        _setAutocomplete: function (value)
        {
            ...
            {
                if (event.keyCode == 38 || event.keyCode == 40 || event.keyCode == 13) // up
																						// 、down、enter
                {
                    return;
                } 
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    setTimeout(function ()
                    {
                        ...
                        /*
						 * 可以在子窗口这样使用： var dialog = frameElement.dialog; var
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        toolbar: null,                           // 工具条,参数同
													// ligerToolbar的,额外参数有title、icon
        ...
        autoFilter: false,                    // 自动生成高级查询,
												// 需要filter/toolbar组件支持.
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("            <div class='l-panel-bbar-inner'");//ylc
			if(p.usePager)//ylc
				gridhtmlarr.push(">");//ylc
			else //ylc
				gridhtmlarr.push(" style='display: none;'>");//ylc
            ...
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:20px' maxlength='10' /> / <span></span></span></div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            ...
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            //ylc begin
            if(p.footbar==null)//ylc
				gridhtmlarr.push("                <div class='l-clear'></div>");//ylc
            gridhtmlarr.push("            </div>");
			if(p.footbar != null){
				gridhtmlarr.push("<div class='l-panel-bbar-inner'>");
				$(p.footbar.items).each(function (i, item){
					if(item.line)
						gridhtmlarr.push("<div class='l-bar-separator'></div>");//ylc
					if(item.text)
						gridhtmlarr.push("<div class='l-bar-button l-bar-btn"+item.icon+"' title='"+item.text+"'><span></span></div>");//ylc
				});
				gridhtmlarr.push("</div>");
			}//ylc end
            ...
            if(p.footbar != null){//ylc
				$(p.footbar.items).each(function (i, item){
					if(item.click)
						$("[title='"+item.text+"']", g.grid).click(item.click);
				});
			}//end ylc
            // 头部
        ...
        _calculateGridBodyHeight: function (h)
        {
            ...
            if (p.title) h -= 24;
            if (p.usePager && p.pagerRender || !p.scrollToPage || p.footbar!=null) //ylc
            	h -= g.toolbar.outerHeight();
            //if (p.usePager || p.footbar!=null) h -= 32;//ylc
        ...
        loadServerData: function (param, clause, sourceType)
        {
            ...
            var ajaxOptions = {
                ...
                {
                	if(p.jsonField!=null)
                		data=data[p.jsonField];//URL取数字段名ylc
        ...
        {
            var g = this, p = this.options;
            if (p.usePager || p.footbar!=null)//ylc
            {
                if (p.pagerRender)
                {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.isrownumber)
                    {
                        ...
                        gridhtmlarr.push('>' + (parseInt((p.page - 1) * p.pageSize + item['__index']) + 1) + '</div></td>');
                        return;
                    }
                    // 如果是复选框(系统列)
                    if (this.ischeckbox)
